...
func Run() {
	...
	rl, err := readline.NewEx(&readline.Config{
		...
		HistoryFile: config.ReplHistoryFile(),
	...
	batch(nil, []string{config.ReplAutoRunFile()})
...
func completionList() []readline.PrefixCompleterInterface {
	...
	pciList = append(pciList, readline.PcItem(commandBatch.Use, comp.FileList(config.ReplBatchDir())...))
...
func batch(cmd *cobra.Command, args []string) {
	...
	for _, file := range args {
		_, err := os.Stat(file)
		if err != nil {
			file = filepath.Join(config.ReplBatchDir(), file)
		}
