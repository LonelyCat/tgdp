...
func CompList() []readline.PrefixCompleterInterface {
	...
	for _, sub := range RootCommand.Commands() {
		pciSub = append(pciSub, readline.PcItem(sub.Use, comp.AvpDataList()...))
	}
...
func add(cmd *cobra.Command, args []string) {
	...
	if err := diameter.AvpValueFromYamlStr(yamlText, -1); err != nil {
		fmt.Println(err)
	}
...
func getGroupedValue(value *diameter.AvpData, shift int) {
	...
	for id := range *store {
		getAvpValues(id, store, shift+padOffset)
	}
}
...
func setAvpValue(args []string) {
	...
	if err := diameter.AvpValueFromYamlStr(yamlData, index); err != nil {
		fmt.Println(err)
	}
}
