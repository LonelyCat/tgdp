...
To run this script execute: tgdp @demo.lua <IP peer> <IMSI>
]]

local d = require("diameter")

if #arg < 2 then
    print(string.format("Usage: %s <IP> <IMSI>", arg[0]))
    return
end

local peer_addr = arg[1]
local imsi = arg[2]
...
local peer_proto = 'tcp'

-- Origing host
--
local org_realm = 'sample.mnc001.mcc999.3gppnetwork.org'
local org_host = 'hss01.sample.mnc001.mcc999.3gppnetwork.org'
local sess_id = org_host

-- Destination host
--
local dest_realm = 'sample.mnc001.mcc999.3gppnetwork.org'
local dest_host = 'mme01.sample.mnc001.mcc999.3gppnetwork.org'
...
idr, err = d.message.fetch(16777251, "ID", true) -- Insert-Subscriber-Data request
if err ~= nil then
    print(err)
    return 1
end
