...
## Current limitations
Multiple AVP values and grouped AVP are not supported.
...
#### `<module>.peer.new(name, address, port, protocol) (peer, error)`
##### Description
...
### Properties
 ...
 `app_id` - message's application id
 `app_name` - message's application name (read-only)
 `cmd_code` - message's command code
 `flags` - message's flags
 `hop_by_hop` - message's "hop by hop" value
 `end_to_end` - message's "end to end" value
