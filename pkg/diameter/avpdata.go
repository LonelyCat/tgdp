...
func LoadAvpsValuesFromYaml(confDir string) error {
	path := filepath.Join(confDir, avpsDataFile)
	...
	node := yaml.Node{}
	err = yaml.Unmarshal(yamlText, &node)
	if err != nil {
	...
	for i := 0; i < len(node.Content[0].Content); i += 2 {
		...
		if err := avpValueFromYamlNode(a, v, -1, nil); err != nil {
			return err
		}
	}
...
func AvpValueFromYamlStr(yamlData string, index int) error {
	node := yaml.Node{}
	...
	a := node.Content[0].Content[0]
	v := node.Content[0].Content[1]

	return avpValueFromYamlNode(a, v, index, nil)
}
...
func avpValueFromYamlNode(avp, value *yaml.Node, index int, store *AvpDataStore) error {
	...
	default:
		fmt.Printf("Unexpected node kind %d at line %d\n", value.Kind, value.Line)
		os.Exit(1)
	}
